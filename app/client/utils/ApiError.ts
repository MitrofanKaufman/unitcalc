...
/**
 * Интерфейс для деталей ошибки API
 */
interface IErrorDetails {
  [key: string]: string | number | boolean | string[] | number[] | IErrorDetails | IErrorDetails[] | null | undefined;
}
...
class ApiError extends Error {
   ...
   */
  details?: IErrorDetails | IValidationError[];
   ...
   */
  static unauthorized(message: string = 'Не авторизован', details?: IErrorDetails): ApiError {
    return new ApiError(
      message,
      401,
   ...
   */
  static forbidden(message: string = 'Доступ запрещен', details?: IErrorDetails): ApiError {
    return new ApiError(
      message,
      403,
   ...
   */
  static notFound(message: string = 'Ресурс не найден', details?: IErrorDetails): ApiError {
    return new ApiError(
      message,
      404,
