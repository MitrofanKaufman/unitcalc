...
export class RouteManager {
  ...
  private async initializeRoutes(): Promise<void> {
    ...
    this.routes.push({
      path: '/api/ping',
      router: (req: Request, res: Response) => res.status(200).json({ 
        status: 'ok', 
    ...
    try {
      ...
      // Поддерживаем оба варианта: /api/v1/products и /api/v1/product для обратной совместимости
      this.routes.push(
        { 
          path: '/api/v1/products', 
          router: productRouter, 
          description: 'API для работы с товарами (множественное число)' 
        },
        { 
          path: '/api/v1/product', 
          router: productRouter, 
          description: 'API для работы с товарами (единственное число - для обратной совместимости)' 
        }
      );
      
      // TODO: Добавить остальные маршруты по мере необходимости
      /*
      const { default: calculatorRouter } = await import('../../api/v1/calculator/calculator.routes');
      const { default: parseRouter } = await import('../../api/v1/parse/parse.routes');
      
      this.routes.push(
        { 
          path: '/api/v1/calculator', 
          router: calculatorRouter, 
          description: 'Калькулятор рентабельности' 
        },
        { 
          path: '/api/v1/parse', 
          router: parseRouter, 
          description: 'API для парсинга товаров' 
        }
      );
      */
