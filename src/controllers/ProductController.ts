...
class ProductController {
   ...
   * @returns Результат анализа товара
   */
  async analyzeProduct(req: Request, res: Response, next: NextFunction) {
    try {
      const { id } = req.params;
      ...
      // Вызываем сервис для анализа товара
      const result = await productService.analyzeProduct(id);
      ...
      return ApiResponse.success(
        res,
        result.data,
        'Анализ товара успешно выполнен'
      );
