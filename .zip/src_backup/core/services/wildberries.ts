...
import { generateImageUrl } from '@/core/services/generateImageUrl.ts';

class WildberriesService {
  static async fetchSuggestions(query: string): Promise<string[]> {
    const res = await fetch(
        `https://suggests.wb.ru/suggests/api/v7/hint?ab_testing=false&query=${encodeURIComponent(query)}&gender=common&locale=kz&lang=ru&appType=1`
    );
    if (!res.ok) throw new Error("WB suggestions error");
  ...
  static async fetchProducts(
    ...
    try {
      ...
      const processedProducts = rawProducts.map((p: any, index: number): Product => {
        ...
        return {
          ...
          image: generateImageUrl(p.id),
          images: p.pics > 0 ? [generateImageUrl(p.id)] : [],
  ...
  static async getProductById(id: string): Promise<Product | null> {
    try {
      const products = await this.fetchProducts(id);
