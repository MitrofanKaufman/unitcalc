 ...
 * Содержит настройки для различных окружений (development, production, test)
 */
...
interface IConfig {
  ...
  security: {
    ...
    passwordResetTokenExpiresIn: number; // в миллисекундах
  };
  paths: {
    root: string;
    src: string;
...
const environmentConfigs: Record<Environment, Partial<IConfig>> = {
  development: {
    env: 'development',
    ...
    logs: {
      ...defaults.logs,
      level: 'debug', // В разработке логируем всё
    },
  },
  production: {
    env: 'production',
    ...
    logs: {
      ...defaults.logs,
      level: 'info', // В продакшене только важные логи
    },
    security: {
      ...defaults.security,
  ...
  test: {
    ...
    logs: {
      ...defaults.logs,
      level: 'error', // В тестах только ошибки
    },
  },
};
