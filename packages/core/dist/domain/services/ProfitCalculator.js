...
export class ProfitCalculator {
     ...
     */
    static calculateProfitability(product, params = {}) {
        const { salesCount = 100, timePeriod = 30, competitorPrices = [], marketDemand = 'medium' } = params;
     ...
     */
    static calculateProfitabilityScore(params) {
        let score = 0;
        // Оценка маржинальности (40% веса)
        if (params.margin >= 50)
            score += 40;
        else if (params.margin >= 30)
            score += 30;
        else if (params.margin >= 15)
            score += 20;
        else if (params.margin >= 5)
            score += 10;
        else
            score += 0;
        // Оценка ROI (30% веса)
        if (params.roi >= 100)
            score += 30;
        else if (params.roi >= 50)
            score += 25;
        else if (params.roi >= 20)
            score += 20;
        else if (params.roi >= 10)
            score += 15;
        else if (params.roi >= 0)
            score += 10;
        else
            score += 0;
        // Оценка точки безубыточности (20% веса)
        if (params.breakEvenPoint <= 10)
            score += 20;
        else if (params.breakEvenPoint <= 25)
            score += 15;
        else if (params.breakEvenPoint <= 50)
            score += 10;
        else if (params.breakEvenPoint <= 100)
            score += 5;
        else
            score += 0;
    ...
    static compareProducts(products) {
        ...
        const results = products.map(product => this.calculateProfitability(product));
        return {
            bestMargin: results.reduce((best, current) => current.margin > best.margin ? current : best).product,
            bestROI: results.reduce((best, current) => current.roi > best.roi ? current : best).product,
            bestScore: results.reduce((best, current) => current.profitabilityScore > best.profitabilityScore ? current : best).product,
            worstMargin: results.reduce((worst, current) => current.margin < worst.margin ? current : worst).product,
            worstROI: results.reduce((worst, current) => current.roi < worst.roi ? current : worst).product,
            worstScore: results.reduce((worst, current) => current.profitabilityScore < worst.profitabilityScore ? current : worst).product
        };
    }
    /**
     * Пакетный расчет доходности
     */
    static calculateBatch(products, params) {
